...
# get just the first two subjects performance data, for a demo
```
...
rating_summary_d = rating_hyp_long_d %>% 
  ...
  summarize(rating_mean = mean(Rating), sem = sd(Rating, na.rm = TRUE)/sqrt(length(Rating)))
  ## your code here
```
...
```{r TamirFig1}
ggplot(rating_summary_d, aes(x=ConfrontationalGame, y=rating_mean, fill=Emotion)) +
  geom_bar(position="dodge", stat="identity") + 
  geom_errorbar(aes(ymin=rating_mean -sem, ymax=rating_mean+sem), width=.2, size=1,position=position_dodge(.9))+
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(MeanImprovementScore = mean(ImprovementScore, na.rm = TRUE), sem = sd(ImprovementScore, na.rm = TRUE)/sqrt(length(ImprovementScore)))
```
...
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position="dodge", stat="identity")+
  geom_errorbar(aes(ymin=MeanImprovementScore-sem, ymax=MeanImprovementScore+sem), width=.2, size=1,position=position_dodge(.9))+
  scale_fill_brewer(palette="Dark2")
```
